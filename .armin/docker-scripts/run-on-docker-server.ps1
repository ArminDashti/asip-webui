...
function Parse-SshTarget([string]$SshValue) {
    ...
    if ($value -match '^(?i)ssh\s+(?<rest>.+)$') {
        ...
        if ($rest -match '^(?<alias>\S+)\s+-p\s+(?<port>\d+)$') {
            $alias = $Matches['alias']
            $port = $Matches['port']
        }
        elseif ($rest -match '^-p\s+(?<port>\d+)\s+(?<alias>\S+)$') {
            $alias = $Matches['alias']
            $port = $Matches['port']
        }
        elseif ($rest -match '^(?<alias>\S+)$') {
            $alias = $Matches['alias']
        }
        else {
            throw 'ssh alias mode must be "ssh <alias>", "ssh <alias> -p <port>", or "ssh -p <port> <alias>".'
        }
...
function Get-SshArgList {
    ...
    [void]$args.Add('-o')
    [void]$args.Add('BatchMode=yes')
    if (-not [string]::IsNullOrWhiteSpace([string]$Target.Port)) {
        [void]$args.Add('-p')
        [void]$args.Add([string]$Target.Port)
    }
    [void]$args.Add($Target.Alias)
    return , $args.ToArray()
}

function Get-ScpArgList {
    param($Target)
    $args = New-Object System.Collections.Generic.List[string]
    [void]$args.Add('-o')
    [void]$args.Add('BatchMode=yes')
...
function Invoke-Remote {
    ...
    if ($Target.Mode -eq 'alias') {
        $sshArgs = Get-SshArgList -Target $Target
...
function Copy-ToRemote {
    ...
    if ($Target.Mode -eq 'alias') {
        $scpArgs = Get-ScpArgList -Target $Target
        & scp @scpArgs $LocalPath "$($Target.Alias):$RemotePath"
...
function Copy-DirToRemote {
    ...
    if ($Target.Mode -eq 'alias') {
        $scpArgs = Get-ScpArgList -Target $Target
        & scp -r @scpArgs "$LocalDir/." "$($Target.Alias):$RemoteDir/"
...
try {
    ...
    $composeExistsCmd = "test -f '$remoteCompose'"

    if ($deleteVolume -or $deleteImage) {
        Write-Step 'Remote compose down (if present)'
        Invoke-Remote -Target $target -RemoteCommand "if $composeExistsCmd; then docker compose -p '$stackName' -f '$remoteCompose' --project-directory '$volumeDir' down $downFlags; fi"
    }
    ...
    if ($buildImageOn -eq 'server') {
        ...
        try {
            ...
            Write-Host "WARN could not remove temp image tar: $($_.Exception.Message)" -ForegroundColor Yellow
        }
